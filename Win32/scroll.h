...
//	사용하실 스크롤 클래스를 정의하세요.
using scroll = class scroll_Win32;
...
public:
	...
	*/
	...
	//	Thumb의 위치에서 임의의 길이만큼 이동합니다. Thumb를 이용하지 않고 스크롤 바를 움직일 때 사용합니다.
	void AdjustnowScrollPosition(float len)
	{
	...
	void SetnowScrollPosition(float len)
	{
		m_fLastGrapPosition = 0;
		m_bGrap = false;
		m_fnowScrollpos = Lus_onlyuse_SideStore(0, len * m_flScroll / m_flSrc, m_flScroll);
	}
...
public:
	...
	*/
	virtual void initialize(SCRType type, float ScrLength, float ScrViewLength, float ScrollLength, POINT WorldPosition,
		int Opacity = 255, float ThumbLength = 0.f, float thickness = 5.0f)
	{
		...
		if (m_scType == SCRType::SCR_H)
			m_rcScroll = RECT{
			static_cast<LONG>(m_ptWorldPosition.x),
			static_cast<LONG>(m_ptWorldPosition.y),
			static_cast<LONG>(m_ptWorldPosition.x + m_flScroll + 2 * m_flThumb),
			static_cast<LONG>(m_ptWorldPosition.y + m_flThickness)
		};
		else
			m_rcScroll = RECT{
			static_cast<LONG>(m_ptWorldPosition.x),
			static_cast<LONG>(m_ptWorldPosition.y),
			static_cast<LONG>(m_ptWorldPosition.x + m_flThickness),
			static_cast<LONG>(m_ptWorldPosition.y + m_flScroll + 2 * m_flThumb)
		};
	...
	*/
	...
	{
		float pos = static_cast<float>((m_scType == SCRType::SCR_H) ? pt.x - m_ptWorldPosition.x : pt.y - m_ptWorldPosition.y);
	...
	*/
	void RenewalThumbRect()
	{
		if (m_scType == SCRType::SCR_H)
			m_rcScroll = RECT{
			static_cast<LONG>(m_ptWorldPosition.x + m_fnowScrollpos),
			static_cast<LONG>(m_ptWorldPosition.y - 1),
			static_cast<LONG>(m_ptWorldPosition.x + 2 * m_flThumb + m_fnowScrollpos),
			static_cast<LONG>(m_ptWorldPosition.y + m_flThickness + 1)
		};
		else
			m_rcScroll = RECT{
			static_cast<LONG>(m_ptWorldPosition.x - 1),
			static_cast<LONG>(m_ptWorldPosition.y + m_fnowScrollpos),
			static_cast<LONG>(m_ptWorldPosition.x + m_flThickness + 1),
			static_cast<LONG>(m_ptWorldPosition.y + 2 * m_flThumb + m_fnowScrollpos)
		};
	}
	...
	*/
	virtual void Draw(HDC hDC, RECT DrawAreaSize, COLORREF color = RGB(0, 0, 0))
	{
		...
		int halfThickness = static_cast<int>(m_flThickness * 0.5f);
		RoundRect(LayDC, m_rcScroll.left, m_rcScroll.top, m_rcScroll.right, m_rcScroll.bottom, halfThickness, halfThickness);

		SetDCPenColor(LayDC, color);
		SetDCBrushColor(LayDC, color);
		SelectObject(LayDC, (HBRUSH)GetStockObject(DC_BRUSH));
		SelectObject(LayDC, (HPEN)GetStockObject(DC_PEN));
		RoundRect(LayDC, m_rcThumb.left, m_rcThumb.top, m_rcThumb.right, m_rcThumb.bottom, halfThickness, halfThickness);
